...
def test_gateway (chrdriver: webdriver.Chrome, path_bootstrap_excel, platform):
    ...
    for exam in df_test_list.keys():
        ...
        do_or_not=df_test_list.iloc[platform][exam]
        if not np.isnan(do_or_not):
            if exam == "carica_esame":
                test_carica_esame.send_more_exams(chrdriver=chrdriver, path_bootstrap_excel=path_bootstrap_excel, platform=platform)
            else:
