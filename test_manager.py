    ...
    for exam in lista_test_platofrom.keys():
        ...
        if not np.isnan(do_or_not):
            if exam == "carica_esame":
                ...
                test_referta_esame.report_more_exams(chrdriver=chrdriver, users=users, config_info=config_info, sended_exam=[1,-1,0,0,1,1,1])#sended_exam da sostituire #[1,0,0,0,1,1,1]
            else:
